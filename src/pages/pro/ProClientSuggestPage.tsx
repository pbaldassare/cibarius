...
import { Loader2, Send, Search, Apple, BookOpen, MessageSquare, Trash2 } from "lucide-react";
    ...
    Promise.all([
      ...
      for (const p of (prodRes.data ?? [])) {
        ...
        results.push({
          ...
          kcal: p.calories_100g ?? 0,
          protein: mac?.protein ?? 0, carbs: mac?.carbs ?? 0, fats: mac?.fats ?? 0,
          source: "product",
        });
      }
      ...
      for (const t of (tmplRes.data ?? [])) {
        ...
        results.push({
          id: t.id, name: t.name,
          kcal: t.calories_100g, protein: t.protein_100g, carbs: t.carbs_100g, fats: t.fats_100g,
          source: "template",
        });
      }
      ...
      for (const i of (ingrRes.data ?? [])) {
        ...
        results.push({
          id: i.id, name: i.name,
          kcal: i.kcal_per_100g, protein: i.protein_per_100g, carbs: i.carbs_per_100g, fats: i.fat_per_100g,
          source: "ingredient",
        });
      }
        ...
        <Tabs defaultValue="food">
          ...
          <TabsContent value="food" className="space-y-3 mt-3">
            ...
            {foodResults.map((f) => (
              <Card
                key={`${f.source}-${f.id}`}
                className={`border cursor-pointer transition-colors ${selectedFood?.id === f.id && selectedFood?.source === f.source ? "border-primary bg-primary/5" : "border-border"}`}
                onClick={() => setSelectedFood(f)}
              >
                <CardContent className="py-2.5">
                  <p className="text-sm font-medium">{f.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {f.brand ? `${f.brand} · ` : ""}{f.kcal} kcal · P {f.protein}g · C {f.carbs}g · G {f.fats}g
                  </p>
                </CardContent>
              </Card>
            ))}
                ...
                <CardContent className="py-3 space-y-2">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <span className="text-xs text-muted-foreground whitespace-nowrap">
                      ≈ {Math.round(selectedFood.kcal * qtyNum / 100)} kcal
                    </span>
                  </div>
          ...
          <TabsContent value="note" className="space-y-3 mt-3">
            ...
            <Textarea
              placeholder="Es: Ricorda di bere almeno 2L di acqua al giorno..."
              value={freeText}
